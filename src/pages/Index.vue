    ...
    .slide(v-show = "step == -1")
      #start
        ...
        .row
          ...
          p
            b 請注意：人的感官接收與認知策略是多元的，這只是認識自己的工具。真正影響學習成效的，並不是固定的風格，而是親師生是否能根據不同任務靈活選擇適切的學習策略。
        ...
        .row
          p 請根據你的學習習慣，選擇最符合你的選項(可複選)。

          //- p 測驗結果將會提示出你最擅長的學習風格，以及如何提升你的學習效率。

          p 您的偏好可能不只一種，可能兩種以上。

          p 對於孩子，可以由家長帶領孩子一起測驗，將題目中的例子改成孩子熟悉的例子，並且由家長陪同孩子一起討論測驗結果。

          p VARK 並非絕對的分類，而是提供一種理解學習偏好的框架，有助於學習者找到可能適合自己的學習方法。
...
export default {
  ...
  methods: {
    ...
    getNum: function () {
      ...
      res = this.countVARK();
      ans = vark.slice().sort(function (a, b) {
        return res[b] - res[a];
      });
      return ans
        .map(function (t) {
    ...
    getAdvice: function () {
      ...
      res = this.countVARK();
      vark = vark.slice().sort(function (a, b) {
        return res[b] - res[a];
      });
      for (i$ = 0, len$ = vark.length; i$ < len$; ++i$) {
        t = vark[i$];
    ...
    getChartSVG: function () {
      ...
      cats.forEach(function (cat) {
        ...
        svg +=
          ...
          '" y="' +
          (cy + 5) +
          '" text-anchor="end" font-size="13" fill="#666">' +
          cat.name +
          '</text>';
        svg +=
          '<rect x="' +
        ...
        svg +=
          ...
          '" y="' +
          (cy + 5) +
          '" font-size="14" font-weight="bold" fill="' +
          cat.color +
          '">' +
          score +
          '</text>';
